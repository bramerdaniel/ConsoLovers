...
public class ShowWithInitModeWhileExecution : MenuBuilderBase
{
   ...
   public void EnsureCommandOnlyWorksWithDefaultOptions()
   {
      ...
      commandNode.Nodes.OfType<ICommandNode>().Where(n => n.IsVisible).Should().HaveCount(0);
   }
   ...
   private IMenuNode[] BuildMenu<T>()
      where T : class
   {
      var options = new MenuBuilderOptions { ArgumentInitializationMode = ArgumentInitializationModes.WhileExecution };
