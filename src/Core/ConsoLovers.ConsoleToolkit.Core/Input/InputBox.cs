...
public class InputBox<T>
{
   ...
   public InputBox(string label, T initialValue)
      : this(label)
   {
   ...
   private InputBox(IConsole console)
   {
   ...
   private string ReadInternal(int length, bool newline)
   {
      ...
      var initialText = InitialValue?.ToString();
      inputRange = new InputRange(initialText, length);

      Console.SetCursorPosition(initialLeft, Console.CursorTop);
      WriteText(initialText ?? string.Empty, length);
